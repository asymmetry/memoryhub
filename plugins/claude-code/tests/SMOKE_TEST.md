...
- [ ] Run `memoryhub-mcp config` yourself in a terminal (it's interactive, so the agent can't drive it) to write your server URL and API token (`mh_...`, minted by an admin via `POST /v1/admin/users/<username>/tokens`) to `<config_dir>/memoryhub/config.toml`. If you skip this, the **check-config** hook reminds you at session start. Confirm the config file was written:

  ```
  cat "$(python3 -c 'import platformdirs; print(platformdirs.user_config_dir())')/memoryhub/config.toml"
  ```
