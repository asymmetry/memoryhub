...
- _Per-user pass_ — group the changed agents by user; for each, `Synthesize(User, prior_summary, sources)` whose sources are the latest per-agent summaries of that user's changed agents, with the user's current summary as `prior_summary`. Per-user reads only per-agent summaries, not raw files. Collect the users that changed.
